...
def getResult(inputSTR, utterance, args, resultDICT, refDICT, pattern="", toolkitDICT={}):
    ...
    if utterance == "I would also want either a 1TB HDD or a 500GB SSD.":
        for arg in args:
            resultDICT["items"].append(arg)
        resultDICT["response"] = getReply2(args, refDICT)
        resultDICT["source"] = "reply"

    if utterance == "I would like it to have 16gb RAM and at least a 512gb SSD drive.":
        for arg in args:
            resultDICT["items"].append(arg)
        resultDICT["response"] = getReply2(args, refDICT)
        resultDICT["source"] = "reply"

    if utterance == "I would most likely consider a Dell laptop or an Asus laptop first.":
        for arg in args:
            resultDICT["items"].append(arg)
        resultDICT["response"] = getReply2(args, refDICT)
        resultDICT["source"] = "reply"

    if utterance == "I would purchase an Apple Macbook pro laptop as a replacement.":
        for arg in args:
            resultDICT["items"].append(arg)
        resultDICT["response"] = getReply2(args, refDICT)
        resultDICT["source"] = "reply"

    if utterance == "I'd like to buy a refurbishment Mac Book Pro.":
        for arg in args:
            resultDICT["items"].append(arg)
        resultDICT["response"] = getReply2(args, refDICT)
        resultDICT["source"] = "reply"

    if utterance == "i would want a touch screen monitor and a mouse pad":
        for arg in args:
            resultDICT["items"].append(arg)
        resultDICT["response"] = getReply2(args, refDICT)
        resultDICT["source"] = "reply"
...
if __name__ == "__main__":
    ...
    resultDICT = getResult("I'd like to buy a refurbishment Mac Book Pro.", "I'd like to buy a refurbishment Mac Book Pro.", [], {}, {})
    pprint(resultDICT)