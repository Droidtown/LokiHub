...
def getReply2(args, refDICT):
    for arg in args:
        arg = purgedPAT.sub("", arg)
        refDICT["goal"].append(arg)
...
def getResult(inputSTR, utterance, args, resultDICT, refDICT, pattern="", toolkitDICT={}):
    ...
    if utterance == "For managing emails and office tasks":
        for arg in args:
            arg = purgedPAT.sub("", arg)
            resultDICT["goal"].append(arg)
        resultDICT["response"] = getReply2(args, refDICT)
        resultDICT["source"] = "reply"

    if utterance == "for VR gaming":
        for arg in args:
            arg = purgedPAT.sub("", arg)
            resultDICT["goal"].append(arg)
        resultDICT["response"] = getReply2(args, refDICT)
    ...
    if utterance == "for accounting software":
        for arg in args:
            arg = purgedPAT.sub("", arg)
            resultDICT["goal"].append(arg)
        resultDICT["response"] = getReply2(args, refDICT)
    ...
    if utterance == "for managing emails":
        for arg in args:
            arg = purgedPAT.sub("", arg)
            resultDICT["goal"].append(arg)
        resultDICT["response"] = getReply2(args, refDICT)
    ...
    if utterance == "for presentations and business meetings":
        for arg in args:
            arg = purgedPAT.sub("", arg)
            resultDICT["goal"].append(arg)
        resultDICT["response"] = getReply2(args, refDICT)
    ...
    if utterance == "for studying and taking online classes":
        for arg in args:
            arg = purgedPAT.sub("", arg)
            resultDICT["goal"].append(arg)
        resultDICT["response"] = getReply2(args, refDICT)
    ...
    if utterance == "for work documents and spreadsheets":
        for arg in args:
            arg = purgedPAT.sub("", arg)
            resultDICT["goal"].append(arg)
        resultDICT["response"] = getReply2(args, refDICT)
    ...
    if utterance == "to run educational apps":
        for arg in args:
            arg = purgedPAT.sub("", arg)
            resultDICT["goal"].append(arg)
        resultDICT["response"] = getReply2(args, refDICT)
...
if __name__ == "__main__":
    ...
    resultDICT = getResult("for studying", "for studying", [], {}, {})
    pprint(resultDICT)