...
def getResult(inputSTR, utterance, args, resultDICT):
    ...
    if utterance == "[50]":
        if re.search("\d+\\.?\d*", args[0]) == None:
            args[0] = re.sub('公斤', '', args[0])
            resultDICT["weight"] = cn2num.transform(args[0])
        elif re.search("\d+\\.?\d*", args[0]) != None:
            resultDICT["weight"] = float(args[0])        
    ...
    if utterance == "[50公斤]":
        if re.search("\d+\\.?\\d*", args[0]) == None:
            ...
            resultDICT["weight"] = list(articut.parse(args[0],level="lv3")["number"].values())[0]
