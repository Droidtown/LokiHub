    ...
    if "抱石" in bts:
        for i in range(len(gymsInfo)):
            if gymsInfo.iloc[i, 3] == 1:
                citySet.add(gymsInfo.iloc[i, 2][:3])
...
def timeToString(timeDict, daySTR):
    ...
    if "平日" in daySTR:
        for gym, time in timeDict.items():
            if time[0] != "n.a.":
                string += gym + "平日營業時間是" + str(time[0]) + "\n"
            else:
                string += gym + "平日不營業哦"
    elif "假日" in daySTR:
        for gym, time in timeDict.items():
            if time[1] != "n.a.":
                string += gym + "假日營業時間是" + str(time[1]) + "\n"
            else:
                string += gym + "假日不營業哦"
        ...
        for gym, time in timeDict.items():
            if time[0] == "預約制":
                ...
                string += gym + "：\n"
                if time[0] != "n.a.":
                    string += "\t平日營業時間 " + str(time[0]) + "\n"
                else:
                    string += "\t平日不營業"
                if time[1] != "n.a.":
                    string += "\t假日營業時間 " + str(time[1]) + "\n"
                else:
                    string += "\t假日不營業"
    return string
...
def getGymDistrict(city, bts):
    ...
    if "抱石" in bts:
        for i in range(len(gymsInfo)):
            if city in gymsInfo.iloc[i,2] and gymsInfo.iloc[i,3] == 1:
                districtSet.add(gymsInfo.iloc[i,2][3:6])
    elif "上攀" in bts or "先鋒" in bts:
        for i in range(len(gymsInfo)):
            if city in gymsInfo.iloc[i,2] and gymsInfo.iloc[i,4] == 1:
                districtSet.add(gymsInfo.iloc[i,2][3:6])
    elif "速度" in bts:
        for i in range(len(gymsInfo)):
            if city in gymsInfo.iloc[i,2] and gymsInfo.iloc[i,5] == 1:
                districtSet.add(gymsInfo.iloc[i,2][3:6])
    else:
        for i in range(len(gymsInfo)):
