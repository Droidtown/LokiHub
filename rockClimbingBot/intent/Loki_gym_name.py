...
def getResult(inputSTR, utterance, args, resultDICT):
    ...
    if utterance == "[中部][上攀][可以]去哪裡":
        if args[1] not in userDefinedDICT["_climbing"]:
            resultDICT["reply_gym_name"] = defaultResponse["_not_rock_climbing"][random.randint(1,4)]
            return resultDICT
        if args[0] in userDefinedDICT["_sides"]:
            ...
            resultDICT["reply_gym_name"] = "{0}的{1}岩館有{2}".format(args[0], args[1], str(selectedGyms))
        elif args[0] in userDefinedDICT["_taiwanAlias"]:
            counties = extendedDICT["_taiwanCities"]
            selectedGyms = getBTSGym(args[1])
            resultDICT["reply_gym_name"] = "{0}的{1}岩館有{2}".format(args[0], args[1], str(selectedGyms))
        else:
            resultDICT["reply_gym_name"] = defaultResponse["_not_taiwan_city"][random.randint(1,4)]
    ...
    if utterance == "[台中][上攀][可以]去哪裡":
        ...
        if len(resultDICT["person_loc"]) != 0:
            ...
            if len(selectedGyms) != 0:
                ...
                resultDICT["reply_gym_name"] = "{0}沒有岩館可以{1}哦！".format(arg[0],args[1])
        else:
            resultDICT["reply_gym_name"] = defaultResponse["_not_taiwan_city"][random.randint(1,4)]
    ...
    if utterance == "[台中][岩館]有哪些":
        ...
        if len(resultDICT["person_loc"]) != 0:
            selectedGyms = getLocBTSGym(resultDICT["person_loc"], args[1])
            if len(selectedGyms) != 0:
                resultDICT["reply_gym_name"] = "{0}的{1}有{2}".format(args[0], args[1], str(selectedGyms))
            else:
                resultDICT["reply_gym_name"] = "{0}沒有{1}哦！".format(arg[0],args[1])
        else:
            resultDICT["reply_gym_name"] = defaultResponse["_not_taiwan_city"][random.randint(1,4)]
    ...
    if utterance == "[台中]攀岩[可以]去哪裡":
        ...
        if len(resultDICT["person_loc"]) != 0:
            ...
            if len(selectedGyms) != 0:
                resultDICT["reply_gym_name"] = "{0}的岩館有{1}".format(args[0], str(selectedGyms))
            else:
    ...
    if utterance == "[台北]哪些[岩館][可以][抱石]":
        ...
        if len(resultDICT["person_loc"]) != 0:
            ...
            if len(selectedGyms) != 0:
                resultDICT["reply_gym_name"] = "{0}的{1}岩館有{2}".format(args[0], args[3], str(selectedGyms))
            else:
                resultDICT["reply_gym_name"] = "{0}沒有{1}岩館哦！".format(arg[0],args[3])
        else:
            resultDICT["reply_gym_name"] = defaultResponse["_not_taiwan_city"][random.randint(1,4)]
    ...
    if utterance == "[台北]哪裡[可以][抱石]":
        ...
        if len(resultDICT["person_loc"]) != 0:
            ...
            if len(selectedGyms) != 0:
                resultDICT["reply_gym_name"] = "{0}的{1}岩館有{2}".format(args[0], args[2], str(selectedGyms))
            else:
                resultDICT["reply_gym_name"] = "{0}沒有{1}岩館哦！".format(arg[0],args[2])
        else:
            resultDICT["reply_gym_name"] = defaultResponse["_not_taiwan_city"][random.randint(1,4)]
    ...
    if utterance == "哪幾[間][岩館]有[速度攀]":
        ...
        if len(selectedGyms) != 0:
            resultDICT["reply_gym_name"] = "{0}的岩館有{1}".format(args[2], str(selectedGyms))
        else:
            resultDICT["reply_gym_name"] = "沒有這種攀岩類型"
    ...
    if utterance == "[東部]哪裡有[速度攀]":
        ...
        if args[0] in userDefinedDICT["_sides"]:
            ...
            if len(selectedGyms) != 0:
                ...
                resultDICT["reply_gym_name"] = "{0}沒有{1}岩館哦".format(args[0], args[1])
        elif args[0] in userDefinedDICT["_taiwanAlias"]:
            counties = extendedDICT["_taiwanCities"]
            selectedGyms = getBTSGym(args[1])
            resultDICT["reply_gym_name"] = "{0}的{1}岩館有{2}".format(args[0], args[1], str(selectedGyms))
