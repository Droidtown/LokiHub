...
def getResult(inputSTR, utterance, args, resultDICT):
    ...
    if utterance == "[中部]哪裡有[上攀][岩館]":
        if args[1] not in userDefinedDICT["_climbing"]:
            resultDICT["reply_gym_location"] = "不認得這種攀岩方式耶"
            return resultDICT
        if args[0] in userDefinedDICT["_taiwanAlias"]:
            ...
            for county in counties:
                if hasGymCounty(county, args[1]):
                    countySet2.add(county)
            if len(countySet2) == 0:
                resultDICT["reply_gym_district"] = "{0}沒有{1}岩館欸".format(args[0], args[1])
            else:
                counties = containerToString(countySet2)
                resultDICT["reply_gym_district"] = "{0}有{1}{2}的地區有{3}".format(args[0], args[1], args[2], counties)                        
        else:
    ...
    if utterance == "[台北]哪裡有[速度攀]": #區、市
        if args[1] not in userDefinedDICT["_climbing"] and args[1] not in userDefinedDICT["_climbingGym"]:
            ...
            return resultDICT
        districtSet = getGymDistrict(args[0], args[1])
        print("districtSet:",districtSet)
        if len(districtSet) > 0:
            district = containerToString(districtSet)
            resultDICT["reply_gym_district"] = "{0}有{1}的地區有{2}".format(args[0], args[1], district)
        else:
            resultDICT["reply_gym_district"] = "{0}沒有{1}".format(args[0], args[1])
