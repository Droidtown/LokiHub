    ...
    if utterance == "[上攀][可以]去哪裡":
        if args[0] in userDefinedDICT["_climbing"]:
            if resultDICT["_person_loc"] != []: #bot 知道人在哪裡
                selectedGyms = getLocBTSGym(resultDICT["_person_loc"], args[0])
                resultDICT["reply_gym_name"] = "{0}的{1}岩館有{2}".format(resultDICT["_person_loc"],args[0], str(selectedGyms))
        else:
            resultDICT["reply_gym_distance"] = random.choice(defaultResponse["_not_rock_climbing"])#defaultResponse["_not_rock_climbing"][random.randint(1,4)]   

    if utterance == "[我]在[台中]":
        if args[0][0] in ("我", "咱"):
            resultDICT["_person_loc"] = args[1]
            resultDICT["reply_gym_distance"] = random.choice(defaultResponse["_msg_received"])

    if utterance == "[我]在[東部]":
        if args[0][0] in ("我", "咱"):
            resultDICT["_person_loc"] = args[1]
            resultDICT["reply_gym_distance"] = random.choice(defaultResponse["_msg_received"])#defaultResponse["_msg_received"][random.choice("_msg_received")]
